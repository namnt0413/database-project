                ...
                <div class="section__content section__content--p30">
                    <div class="container-fluid">
                        ...
                        if (isset($_GET['id']) && !empty($_GET['id'])) {
                            ...
                            $result = mysqli_query($con, "DELETE FROM `books_publishers` WHERE `book_id` = $book_id AND `publisher_id` = ".$_GET['publisher_id']); 
                            // var_dump($result);exit; 
                            if (!$result) {
                                $error = "Không thể xóa NXB cuốn sách này.";
                            }
                            mysqli_close($con);
                            if ($error !== false) {
                                ?>
                            ...
                            <?php } else { ?>
                                <div id="success-notify" class="box-content">
                                    <h2>Xóa NXB cuốn sách thành công</h2>
                                    <a href="javascript:window.history.go(-1)">Quay lại</a>
                                </div>
                            <?php } ?>
                        <?php } ?>
