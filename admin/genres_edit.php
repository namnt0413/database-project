   ...
   <body>      
    ...
    <?php 
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'edit') 
        {
            ...
            if ( isset($_POST['name']) && !empty($_POST['name']) ) 
            {
                ...
                if ($error !== false) 
                {
                    ...
                    <div id="error-notify" class="box-content">
                        <h1>Thông báo</h1>
                        <h4><?= $error ?></h4>
                        <a href="javascript:window.history.go(-1)">Danh sách thể loại</a>
                    </div>
                <?php } else { ?>
                    <div class="content-container">
                        <div id="edit-notify" class="box-content">
                            <h1><?= ($error !== false) ? $error : "Sửa thể loại thành công" ?></h1>
                            <a class="link-button" href="javascript:window.history.go(-2)">Danh sách thể loại</a>
                        </div>
                    </div>
                <?php } ?>
            <?php } else { ?>
                <div class="content-container">
                    <div id="edit-notify" class="box-content">
                        <h1>Vui lòng nhập đủ thông tin để sửa thể loại</h1>
                        <a class="button" href="javascript:window.history.go(-1)">Quay lại sửa thể loại</a>
                    </div>
                </div>
            <?php
            }
            ...
            $result = mysqli_query($con, "SELECT * FROM genres where `id`=" . $_GET['id']);
            ...
            if (!empty($user)) {
                ...
                <div class="content-container">
                    <div id="edit_user" class="box-content">
                        ...
                        <h1>Sửa thể loại "<?= $genres['name'] ?>"</h1>
                        <form action="./genres_edit.php?action=edit" method="Post" enctype="multipart/form-data" autocomplete="off">  
                            <div class="input-block">
                                <input class="input-area" type="hidden" name="id" value="<?= (!empty($genres) ? $genres['id'] : "") ?>" />
                            </div>
                            <div class="input-block">
                                <label>Tên</label></br>
                                <input class="input-area" type="text" name="name" value="<?= (!empty($genres) ? $genres['name'] : "") ?>" />
                            </div>
