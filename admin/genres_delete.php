   ...
   <body>      
    ...
    ?>
        ...
        if (isset($_GET['id']) && !empty($_GET['id'])) {
            ...
            $result = mysqli_query($con, "DELETE FROM `genres` WHERE `id` = " . $_GET['id']);
            if (!$result) {
            ...
            if ($error !== false) {
                ...
                <div class="content-container">
                    <div id="error-notify" class="box-content">
                        <h1>Thông báo</h1>
                        <h4><?= $error ?></h4>
                        <a class="link-button" href="javascript:window.history.go(-1)">Danh sách thể loại</a>
                    </div>
                </div>
            <?php } else { ?>
                <div class="content-container">
                    <div id="success-notify" class="box-content">
                        <h1>Xóa thể loại thành công</h1>
                        <a class="link-button" href="javascript:window.history.go(-1)">Danh sách thể loại</a>
                    </div>
                </div>
            <?php } ?>
