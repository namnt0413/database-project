    ...
    ?>
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'create') {
            ...
            // POST nhan dc day du username va password
                ...
                $result = mysqli_query($con, "INSERT INTO `genres` (`id`, `name`, `last_updated`) VALUES (NULL, '" . $_POST['name'] . "' , '" . time() . "');");
                // neu nhu ko insert dc : trung ten user
                if (!$result) {
                    if (strpos(mysqli_error($con), "Duplicate entry") !== FALSE) {
                        $error = "Thể loại này đã tồn tại. Bạn vui lòng nhập thể loại khác.";
                    }
                }
                mysqli_close($con);
                ...
                if ($error !== false) {
                    ...
                    <div class="content-container">
                        <div id="error-notify" class="box-content">
                            <h1>Thông báo</h1>
                            <h4><?= $error ?></h4>
                            <a href="./genres_create.php" class="link-button">Tạo thể loại khác</a>
                        </div>
                    </div>
                <?php } else { ?>
                    <div class="content-container">
                        <div id="success-notify" class="box-content">
                            <h1>Chúc mừng</h1>
                            <h4>Bạn đã tạo thành công thể loại <?= $_POST['name']?></h4>
            ...
            <?php }  else { ?>
                <div class="content-container">
                    <div id="edit-notify" class="box-content">
                        <h1>Vui lòng nhập đủ thông tin để thêm thể loại</h1>
                        <a class="button" href="javascript:window.history.go(-1)">Quay lại thêm thể loại</a>
                    </div>
                </div>
            <?php } ?>   
        <?php } else { ?>   
            ...
            <a href="javascript:window.history.go(-1)" class="fa fa-undo" style="padding: 5px; margin-bottom: 10px;">  Quay lại</a>       
                <div id="create_user" class="box-content">
                    <h1>Tạo thể loại</h1>
                    <form action="./genres_create.php?action=create" method="Post" autocomplete="off">
                        <br>
                        ...
                        <br><br>
                        <input class="button "  type="submit" value="Tạo thể loại" />
                    </form> 
                </div>
            </div>
        <?php } ?>
